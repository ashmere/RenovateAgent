...
class GitHubClient:
    ...
    async def _authenticate(self) -> None:
        ...
        try:
            ...
            pat = getattr(self.config, 'github_personal_access_token', None) or getattr(self.config, 'personal_access_token', None)
            if pat:
            ...
            access_token = await self._get_installation_access_token(jwt_token, installation_id)

            # Create authenticated GitHub instance
            self._github = Github(access_token)
            self._installation_id = installation_id

            logger.info("GitHub authentication successful (GitHub App mode)",
    ...
    async def _get_installation_id(self, jwt_token: str) -> int:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                raise AuthenticationError(f"Failed to get installations: {response.text}")

            installations = response.json()

            for installation in installations:
                if installation.get('account', {}).get('login') == self.config.organization:
                    return installation['id']

            raise AuthenticationError(f"No installation found for organization: {self.config.organization}")

    async def _get_installation_access_token(self, jwt_token: str, installation_id: int) -> str:
        """Get installation access token."""
        async with httpx.AsyncClient() as client:
    ...
    def _update_rate_limit_info(self, github_instance) -> None:
        ...
        try:
            ...
            if isinstance(github_instance, Github):
                ...
                github_obj = github_instance._requester._Github__requester if hasattr(github_instance, '_requester') else None
                if github_obj:
    ...
    async def get_pr_checks(self, repo: Repository, pr: PullRequest) -> List[CheckRun]:
        ...
        try:
            ...
            logger.info("Retrieved PR checks",
                       repo=repo.full_name,
                       pr_number=pr.number,
    ...
    async def approve_pr(self, repo: Repository, pr_number: int,
                        review_body: str = "Auto-approved by Renovate PR Assistant") -> bool:
        """
    ...
    async def create_issue(self, repo: Repository, title: str, body: str,
    ...
    async def update_issue(self, repo: Repository, issue_number: int,
                          title: Optional[str] = None,
                          body: Optional[str] = None,
                          state: Optional[str] = None) -> Issue:
        """
    ...
    async def find_issue_by_title(self, repo: Repository, title: str) -> Optional[Issue]:
        """
        ...
        try:
            ...
            for issue in issues:
                if issue.title == title:
                    logger.info("Found issue by title",
                               repo=repo.full_name,
                               issue_number=issue.number,
                               title=title)
                    return issue

            logger.info("Issue not found by title",
                       repo=repo.full_name,
                       title=title)
            return None
        except GithubException as e:
            logger.error("Failed to search for issue",
                        repo=repo.full_name,
                        title=title,
                        error=str(e))
            raise GitHubAPIError(f"Failed to search for issue: {e}")

    async def commit_file(self, repo: Repository, file_path: str,
                         content: str, message: str, branch: str = "main") -> bool:
        """
    ...
    async def get_organization_repositories(self, org_name: str, 
        ...
        try:
            ...
            logger.error("Failed to get organization repositories",
                        org=org_name,
                        error=str(e))
