...
class DependencyFixerFactory:
    ...
    def _initialize_fixers(self) -> None:
        ...
        if "python" in self.settings.supported_languages:
            self.fixers.append(PythonPoetryFixer(
                timeout=self.settings.dependency_update_timeout
            ))

        if "typescript" in self.settings.supported_languages:
            self.fixers.append(TypeScriptNpmFixer(
                timeout=self.settings.dependency_update_timeout
            ))
        ...
        if "go" in self.settings.supported_languages:
            self.fixers.append(GoModFixer(
                timeout=self.settings.dependency_update_timeout
            ))

        logger.info("Dependency fixers initialized",
    ...
    async def get_fixer(self, repo: Repository) -> Optional[DependencyFixer]:
        ...
        if not self.settings.enable_dependency_fixing:
            logger.info("Dependency fixing disabled",
        ...
        try:
            ...
            for fixer in self.fixers:
                if await self._can_fixer_handle_repo(fixer, repo_info):
                    logger.info("Selected dependency fixer",
                               repository=repo.full_name,
                               fixer=fixer.__class__.__name__)
                    return fixer

            logger.info("No suitable dependency fixer found",
                       repository=repo.full_name)
            return None

        except Exception as e:
            logger.error("Failed to get dependency fixer",
                        repository=repo.full_name,
                        error=str(e))
            raise DependencyFixingError(f"Failed to get dependency fixer: {e}")

    async def _analyze_repository(self, repo: Repository) -> dict:
        """
        ...
        try:
            ...
            analysis = {
                ...
                "has_typescript": "tsconfig.json" in files or any(f.endswith('.ts') for f in files),
    ...
    async def _can_fixer_handle_repo(self, fixer: DependencyFixer,
                                   repo_info: dict) -> bool:
        """
        ...
        try:
            ...
            if isinstance(fixer, PythonPoetryFixer):
                return repo_info.get("has_poetry", False)

            elif isinstance(fixer, TypeScriptNpmFixer):
                return (repo_info.get("has_npm", False) and
                       (repo_info.get("has_typescript", False) or
                        repo_info.get("has_javascript", False)))

            elif isinstance(fixer, GoModFixer):
                return repo_info.get("has_go", False)

            return False

        except Exception as e:
            logger.error("Failed to check fixer compatibility",
                        fixer=fixer.__class__.__name__,
                        error=str(e))
            return False
    ...
    async def get_fixer_info(self) -> List[dict]:
        ...
        for fixer in self.fixers:
            try:
                ...
                logger.error("Failed to get fixer info",
                           fixer=fixer.__class__.__name__,
                           error=str(e))
