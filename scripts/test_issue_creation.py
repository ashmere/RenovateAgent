        ...
        try:
            ...
            if dashboard_issue:
                ...
                renovate_prs = [
                    ...
                    for pr in open_prs
                    if (
                        "renovate" in pr.user.login.lower()
                        or "dependabot" in pr.user.login.lower()
                    )
                ]
                stats = {
                ...
                print("   ❌ Failed to create dashboard issue")
    ...
    for repo_name in target_repos:
        try:
            ...
            for pr in open_prs[:3]:  # Analyze first 3 PRs
                ...
                if is_renovate:
                    print("      🤖 Renovate PR - would be processed!")
                elif is_dependabot:
                    print("      🤖 Dependabot PR - would be processed!")
                else:
                    print("      👤 Human PR - would be ignored")
...
async def main():
    ...
    # Confirm with user
    response = input(
        "Do you want to create dashboard issues in your repositories? (y/N): "
    )
    if response.lower() != "y":
    ...
    try:
        ...
        if dashboard_results:
            ...
            for result in dashboard_results:
                ...
                print(
                    f"      Stats: {result['stats']['total_open_prs']} open PRs, {result['stats']['renovate_prs']} dependency PRs"
                )

        print("\n💡 Next steps:")
        print("   1. Check the issues created in your repositories")
        print("   2. Create a test PR to see the system in action")
        print("   3. Configure real webhooks for live processing")
